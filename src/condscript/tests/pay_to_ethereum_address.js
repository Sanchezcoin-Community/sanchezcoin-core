...
async function eth_sig_true_test() {
    ...
    equal_spefic_signature_pkey(EthAddress(0x2a627c97c15c43Fa7692E9886EB805c8AfA70DfB));
    unlock();
    exit();
    `
...
// Wird verwendet um zu überprüfen ob Skripte welche eine Ethereum Signatur verwenden korrekt sind
async function eth_sig_true_test2() {
    // Wird verwendet um eine Ausgabe an bestimmte bedinungen zu knüpfen
    let eth_sig_locking_script = `
    if(is_a_signer(EthAddress(0x2a627c97c15c43Fa7692E9886EB805c8AfA70DfB)) == true) {
        unlock();
        exit();
    }
    `
    ...
    let test_result = await runScript(tx_check_data, tx_chain_data, null, false);
    if(use_debug_mode === true) console.log()
    console.log('Check validate web3 signature 2:', test_result.isFinallyTrue());
};

// Wird verwendet um zu überprüfen ob Skripte welche eine Ethereum Signatur verwenden fehlerhaft sind
async function eth_sig_false_test2() {
    // Wird verwendet um eine Ausgabe an bestimmte bedinungen zu knüpfen
    let eth_sig_locking_script = `
    if(is_a_signer(EthAddress(0x2a627c97c15c43Fa7692E9886EB805c8AfA70DfB)) == true) {
        unlock();
    ...
    let test_result = await runScript(tx_check_data, tx_chain_data, null, false);
    if(use_debug_mode === true) console.log()
    console.log('Check invalid web3 signature 2:', test_result.isFinallyTrue() === false);
};


(async() => {
    await eth_sig_true_test();
    await eth_sig_true_test2();
    await eth_sig_false_test2();
