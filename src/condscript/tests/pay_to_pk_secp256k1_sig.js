...
const { parseScript, runScript, getPayToPKeySecp256k1 } = require('../src/index');
...
async function eth_sig_true_test() {
    ...
    let keypair = blockchain_crypto.ecc.secp256k1.getKeyPairFromSeed(seed, '0/0/2');
    let sigk = '654924d66101913f317fe6f965de8ea67b13e6be6b0c804e8098e86ccafbd502';
    let fsig = await blockchain_crypto.ecc.secp256k1.signDigest(keypair.private_key, sigk);
    ...
    add_verify_key(PublicKey(secp256k1, ${keypair.public_key}));
    verify_sig();
    exit();
    `
    ...
    let eth_sig_avail_sigs = [
        new SingleSignatureValue(
            keypair.public_key, 'secp256k1', fsig, sigk
        ),
    ];

    // Die Skripte werden in Hexcode umgewandelt
    let p_locking_script = await getPayToPKeySecp256k1(keypair.public_key);
